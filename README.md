#Internship Application Form 📋

This repository contains the Google Form created for collecting applications for an internship program. The form is designed to gather essential details from applicants such as personal information, educational background, skills, and availability.

📌 Purpose

The main objective of this form is to:

Collect internship applications efficiently

Standardize applicant information

Simplify the screening and shortlisting process

🔗 Google Form Link

👉 Click here to access the Internship Application Form

https://codebybhumika.github.io/Internship_Form/

🛠️ How to Use

1. Open the Google Form using the link above

2. Fill in all the required details

3. Submit the form

📄 License

This project is open for educational and internal use.
